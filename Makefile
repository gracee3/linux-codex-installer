...
.PHONY: help latest status install uninstall source-install lint config
...
help:
	...
	@echo "  source-install  Sync $(SOURCE_DIR) to latest release source tag"
	...
	@echo "  SOURCE_DIR        Source directory for codex checkout (default: $(HOME)/git/codex)"
	@echo ""
	@echo "Examples:"
	@echo "  make latest"
	@echo "  make status"
...
source-install:
	@"$(SOURCE_INSTALL_SCRIPT)" --dir "$(SOURCE_DIR)"

lint:
	@bash -n $(SCRIPT) $(SOURCE_INSTALL_SCRIPT)
